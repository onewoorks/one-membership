...
$app->post('/hadiah/add', function($request, $response, $args) {
    ...
    print_r($input);
    $sql = "INSERT INTO `ganjaran` (`nama_ganjaran`, `mata`, `kuantiti`, `user`, `tarikh_daftar`, `image`, `jenis_hadiah`, `domain`, `nilai`) VALUES (:nama_ganjaran,:mata,:kuantiti,:user,:tarikh_daftar,:image, :jenis_hadiah, :domain, :nilai)";
    ...
    $sth->bindParam(':nilai', isset($input['nilai']) ? $input['nilai'] : 0.00);
    $sth->execute();
});
